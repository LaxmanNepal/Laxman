...
function log(event) {
  audit.push({id:crypto.randomUUID(), ts:new Date().toISOString(), ...event});
  if (audit.length > 500) audit.shift();
}
function auth(req) {
  if (!token) return process.env.NODE_ENV === 'test';
  const h = req.headers.authorization || '';
  return safeEqual(h.startsWith('Bearer ') ? h.slice(7) : '', token);
}
function body(req) {
  return new Promise((resolve,reject)=>{
    let data=''; let size=0;
    req.on('data', c=>{ size += c.length; if(size > policy.max_body_bytes) { reject(new Error('body_too_large')); req.destroy(); } else data += c; });
...
async function handle(req,res){
  ...
  if(req.method==='GET' && url.pathname==='/health') return json(res,200,{ok:true,service:'laxman-super-phone',version:'10.0',time:new Date().toISOString()});
  ...
  if(req.method==='POST' && url.pathname==='/command'){
    ...
    const agent=agentFor(spec.agent);
    if(!agent || !spec.action) return json(res,400,{error:'unknown_agent_or_action'});
    if(forbidden(spec.action)) return json(res,403,{error:'blocked_action'});
    const risk=spec.risk || agent.risk || 'L3';
    if(requiresApproval(risk)){
      const id=crypto.randomUUID(); const item={id,agent:agent.id,action:spec.action,args:spec.args||c.args||{},risk,createdAt:Date.now(),expiresAt:Date.now()+policy.approval_ttl_seconds*1000};
      approvals.set(id,item); log({type:'approval_requested',...item});
      return json(res,202,{status:'approval_required',approval_id:id,expires_at:new Date(item.expiresAt).toISOString()});
    }
    log({type:'command_executed',agent:agent.id,action:spec.action,risk,source:c.source||'api'});
    return json(res,200,{status:'accepted',agent:agent.id,action:spec.action,risk,args:spec.args||c.args||{}});
  }
  const m=url.pathname.match(/^\/approval\/([^/]+)$/);
  if(req.method==='POST' && m){
    const item=approvals.get(m[1]); if(!item || item.expiresAt<Date.now()) return json(res,404,{error:'approval_not_found_or_expired'});
    const c=await body(req); const decision=c.decision;
    if(!['approve','deny'].includes(decision)) return json(res,400,{error:'decision_must_be_approve_or_deny'});
    approvals.delete(item.id); log({type:`approval_${decision}d`,...item});
    return json(res,200,{status:decision,approval_id:item.id});
  }
